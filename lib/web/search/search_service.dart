...
class SearchService {
  ...
  /// Último error de red, para poder explicárselo al usuario en vez de
  /// mostrarle una lista vacía sin motivo.
  String? lastError;

  // Usamos un cliente estático para mantener cookies y sesión entre peticiones
  static final http.Client _client = http.Client();

  /// En navegador, `User-Agent` es una cabecera prohibida y `Cache-Control` o
  /// `Pragma` disparan una petición previa de comprobación que el servidor
  /// remoto rechaza. Fijarlas hace fallar la petición entera, así que en web
  /// se envían solo las que el navegador permite.
  Map<String, String> get _headers => kIsWeb
      ? const {'Accept-Language': 'es-ES,es;q=0.9'}
      : const {
          'User-Agent':
  ...
  Future<http.Response> _get(String url) {
    return _client
        .get(CorsProxy.wrap(Uri.parse(url)), headers: _headers)
    ...
    try {
      final effectiveSource =
          ...
          (urlString.contains('cookpad.com')
              ...
              : RecipeSource.lidl);
      final response = await _get(urlString);
      ...
      lastError = 'La web de recetas respondió ${response.statusCode}.';
    } catch (e) {
      debugPrint('Excepción durante el scraping de búsqueda: $e');
      lastError = CorsProxy.failureHint;
    }
    return [];
  }
  ...
  Future<Recipe?> getFullRecipe(String url) async {
    ...
    try {
      ...
      // Lidl a veces devuelve la página sin los pasos al primer intento.
      ...
      debugPrint('Error al obtener la receta completa: $e');
      lastError = CorsProxy.failureHint;
    }
    return null;
  }
  ...
  Recipe? _parseFullRecipeDetails(String htmlContent) {
    try {
      ...
      debugPrint('Error parseando detalle de receta: $e');
      return null;
    }
  }
  ...
  Recipe? _parseCookpadRecipeDetails(String htmlContent) {
    try {
      ...
      debugPrint('Error parseando detalle de receta Cookpad: $e');
      return null;
    }
  }
  ...
  List<WebRecipeResult> _parseCookpadSearch(String htmlContent) {
    ...
    for (var item in items) {
      try {
        ...
        debugPrint('Error parseando tarjeta Cookpad: $e');
      }
    }
    return results;
  }
  ...
  List<WebRecipeResult> _parseLidlDom(String htmlContent) {
    ...
    for (var card in cards) {
      try {
        ...
        debugPrint('Error parseando tarjeta: $e');
      }
    }
    return results;
  }
